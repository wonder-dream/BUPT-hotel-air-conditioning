...
│
├── tests/                      # 测试脚本
│   ├── test_heating.py        # 制热模式测试
│   ├── test_cooling.py        # 制冷模式测试
│   ├── test_scheduler.py      # 调度器测试
│   ├── generate_room_report.py # 房间报告生成
│   ├── check_db.py            # 数据库检查
│   └── data/                  # 测试数据（Excel）
│
├── document/                   # 需求文档
...
- 每次用户**主动开机**（从关机状态）算作住一天
- 达到目标温度后待机，温度回升自动重启**不计入**开机次数
- 从等待队列调度到服务队列**不计入**开机次数
- 至少收取 1 天房费
...
AccommodationOrder(入住订单)
  ...
  ├── room_fee              # 房费
  ├── deposit_amount        # 押金
  ├── deposit_paid          # 押金是否已收取
  └── power_on_count        # 空调开机次数（用于计算房费天数）
...
COOLING_MIN_TEMP = 15      # 制冷最低温度（后端校验）
COOLING_MAX_TEMP = 30      # 制冷最高温度（后端校验）
HEATING_MIN_TEMP = 20      # 制热最低温度（后端校验）
HEATING_MAX_TEMP = 30      # 制热最高温度（后端校验）
TEMP_THRESHOLD = 1         # 温度偏离阈值
INITIAL_ROOM_TEMP = 28     # 初始室温
...
# 时间缩放比例
TIME_SCALE = 6             # 系统时间 = 真实时间 × 6
```

### 前端温控范围

前端 `CustomerPanel.vue` 中定义了用户可调节的温度范围：
...
| 脚本 | 功能 |
|------|------|
| `test_heating.py` | 制热模式自动化测试 |
| `test_cooling.py` | 制冷模式自动化测试 |
| `test_scheduler.py` | 调度器单元测试 |
| `generate_room_report.py` | 生成房间测试报告 |
| `check_db.py` | 数据库检查工具 |

测试数据位于 `tests/data/` 目录（Excel 格式）。
